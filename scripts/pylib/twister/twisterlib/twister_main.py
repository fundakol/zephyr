...
class Twister:
    ...
    def export_previous_results(self, previous_results: str | None) -> str | None:
        """Export previous results to a file and return path to the file."""
...
def main(options: argparse.Namespace, default_options: argparse.Namespace) -> int:
    """Run twister."""
    try:
        return_code = Twister(options, default_options).run()
    finally:
        close_logging()
    return return_code
