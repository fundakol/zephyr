...
if sys.platform == 'linux':
    from twister.twisterlib.jobserver import (
        GNUMakeJobClient,
        GNUMakeJobServer,
        JobClient,
        JobHandle
...
def test_gnumakejobclient_from_environ(
    ...
    with mock.patch('fcntl.fcntl', mock_fcntl), \
         ...
         mock.patch('twister.twisterlib.jobserver.GNUMakeJobClient.__init__',
                    gmjc_init_mock), \
...
def test_gnumakejobclient_pass_fds(jobs, use_inheritable_pipe, expected_fds):
    ...
    with mock.patch('twister.twisterlib.jobserver.GNUMakeJobClient.__del__',
                    mock.Mock()), \
         mock.patch('selectors.DefaultSelector', selector_mock):
        deleter()
